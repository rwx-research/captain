...
var _ = Describe("BuildkiteProvider.GetBranchName", func() {
	It("prefers the git repository branch over the env branch", func() {
		provider := providers.BuildkiteProvider{
			Env: providers.BuildkiteEnv{
				BuildkiteBranch: "main",
			},
		...
		Expect(provider.GetBranchName()).To(Equal("other"))
	})

	It("falls back to the env when the git repository has no branch", func() {
		provider := providers.BuildkiteProvider{
			Env: providers.BuildkiteEnv{
				BuildkiteBranch: "main",
			},
...
var _ = Describe("BuildkiteProvider.GetCommitSha", func() {
	It("prefers the git repository branch over the env branch", func() {
		provider := providers.BuildkiteProvider{
			Env: providers.BuildkiteEnv{
				BuildkiteCommit: "abcd",
			},
			GitRepository: struct {
				Branch        string
				CommitSha     string
		...
		Expect(provider.GetCommitSha()).To(Equal("efgh"))
	})
	...
	It("falls back to the env when the git repository has no branch", func() {
		...
		Expect(provider.GetCommitSha()).To(Equal("abcd"))
	})
})

var _ = Describe("BuildkiteProvider.GetCommitMessage", func() {
	It("prefers the git repository branch over the env branch", func() {
		provider := providers.BuildkiteProvider{
			Env: providers.BuildkiteEnv{
				BuildkiteMessage: "message",
			},
			GitRepository: struct {
				Branch        string
				CommitSha     string
		...
		Expect(provider.GetCommitMessage()).To(Equal("other"))
	})
	...
	It("falls back to the env when the git repository has no branch", func() {
		...
		Expect(provider.GetCommitMessage()).To(Equal("message"))
	})
})
