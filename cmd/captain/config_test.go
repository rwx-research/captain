...
var _ = Describe("InitConfig", func() {
	...
	Context("with GitLab environment variables", func() {
		...
		It("parses the GitLab options", func() {
			...
			Expect(cfg.GitLab.JobStage).To(Equal("test"))
			Expect(cfg.GitLab.JobID).To(Equal("3889399980"))
			Expect(cfg.GitLab.PipelineID).To(Equal("798778026"))
			Expect(cfg.GitLab.JobURL).To(Equal("https://gitlab.com/captain-examples/rspec/-/jobs/3889399980"))
			Expect(cfg.GitLab.PipelineURL).To(Equal("https://gitlab.com/captain-examples/rspec/-/pipelines/798778026"))
			Expect(cfg.GitLab.NodeTotal).To(Equal("2"))
			Expect(cfg.GitLab.NodeIndex).To(Equal("1"))
			Expect(cfg.GitLab.UserLogin).To(Equal("michaelglass"))
			Expect(cfg.GitLab.ProjectPath).To(Equal("captain-examples/rspec"))
			Expect(cfg.GitLab.ProjectURL).To(Equal("https://gitlab.com/captain-examples/rspec"))
			Expect(cfg.GitLab.CommitSHA).To(Equal("03d68a49ef1e131cf8942d5a07a0ff008ede6a1a"))
			Expect(cfg.GitLab.CommitAuthor).To(Equal("Michael Glass <me@mike.is>"))
